...
import 'package:fitnessapp/presentation/screens/my_medicine/add_medicine_screen.dart';
...
class _ViewMedicineScreen extends State<ViewMedicineScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    _medicineList.where((medicine) => !medicine.offStatus! && !medicine.isDeleted!).toList();
    List<MedicineModel> inactiveMedicines =
    _medicineList.where((medicine) => medicine.offStatus! && !medicine.isDeleted!).toList();
              ...
              child: ElevatedButton.icon(
                onPressed: () async {
                  await Navigator.push(
                    context,
                    RouteHelper.createFadeRoute(
                      AddMedicineScreen(idUser: widget.idUser),
                    ),
                  );
                  _fetchMedicines();
                },
  ...
  Future<void> _navigateToEditMedicine(BuildContext context, MedicineModel medicine) async {
    try {
      ...
      Navigator.pushReplacement(
        context,
        RouteHelper.createFadeRoute(EditMedicineScreen(medicineInfo: fetchedMedicineInfo)),
      );
    } catch (e) {
